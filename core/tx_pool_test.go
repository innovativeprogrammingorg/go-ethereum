...
func TestTransactionTotalSizeLimiting(t *testing.T) {
	...
	config := testTxPoolConfig
	config.TotalPoolSizeLimit = common.StorageSize(10 * 1000)
	...
	if pool.all.TotalSize() > config.TotalPoolSizeLimit {
		t.Fatalf("Pool size too large: %d. Expected at most %d", uint64(pool.all.TotalSize()), uint64(config.TotalPoolSizeLimit))
	}
	if err := validateTxPoolInternals(pool); err != nil {
		t.Fatalf("pool internal state corrupted: %v", err)
	}
